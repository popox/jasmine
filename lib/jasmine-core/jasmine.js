...
jasmine.Spec.prototype.addBeforesAndAftersToQueue = function() {
  ...
  for (i = 0; i < this.afterCallbacks.length; i++) {
    this.queue.addAfter(new jasmine.Block(this.env, this.afterCallbacks[i], this), true);
  }
  for (suite = this.suite; suite; suite = suite.parentSuite) {
    for (i = 0; i < suite.after_.length; i++) {
      this.queue.addAfter(new jasmine.Block(this.env, suite.after_[i], this), true);
    }
  }
  for (i = 0; i < runner.after_.length; i++) {
    this.queue.addAfter(new jasmine.Block(this.env, runner.after_[i], this), true);
  }
};
